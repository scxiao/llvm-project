...
//===----------------------------------------------------------------------===//
// ToyToAffine RewritePatterns: AddOne operations
//===----------------------------------------------------------------------===//
// The add one operation has a constant of value 1 and the 
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    lowerOpToLoops(op, operands, rewriter,
                   [&, loc](OpBuilder &builder, ValueRange memRefOperands,
                         ValueRange loopIvs) {
                     // Generate an adaptor for the remapped operands of the
                     // AddOneOp. This allows for using the nice named
                     // accessors that are generated by the ODS.
                     toy::AddOneOpAdaptor addOneAdaptor(memRefOperands);
                     Value input = addOneAdaptor.getInput();
                     auto arg = builder.create<AffineLoadOp>(loc, input,
                                                         loopIvs);
                     return builder.create<arith::AddFOp>(loc, arg, const1);
                   });
    return success();
  }
...
void ToyToAffineLoweringPass::runOnOperation() {
  ...
  patterns.add<AddOpLowering, ConstantOpLowering, FuncOpLowering, MulOpLowering,
               PrintOpLowering, ReturnOpLowering, TransposeOpLowering, AddOneOpLowering>(
      &getContext());
